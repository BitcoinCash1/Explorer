...
import { BlockExtended, BlockSummary, PoolTag, TransactionExtended, TransactionStripped, TransactionMinerInfo } from '../mempool-bch.interfaces';
import { Common } from './common-bch';
import diskCache from './disk-cache';
import transactionUtils from './transaction-utils-bch';
import bitcoinClient from './bitcoin-cash/bitcoin-client';
import { IBitcoinApi } from './bitcoin-cash/bitcoin-api.interface';
import { IEsploraApi } from './bitcoin-cash/esplora-api.interface';
import poolsRepository from '../repositories-cash/PoolsRepository';
import blocksRepository from '../repositories-cash/BlocksRepository';
import loadingIndicators from './loading-indicators';
import BitcoinApi from './bitcoin-cash/bitcoin-api';
import { prepareBlock } from '../utils/blocks-bch-utils';
import BlocksRepository from '../repositories-cash/BlocksRepository';
import HashratesRepository from '../repositories-cash/HashratesRepository';
import indexer from '../indexer-bch';
import fiatConversion from './fiat-conversion-bch';
import poolsParser from './pools-parser-bch';
import BlocksSummariesRepository from '../repositories-cash/BlocksSummariesRepository';
import BlocksAuditsRepository from '../repositories-cash/BlocksAuditsRepository';
import mining from './mining-cash/mining';
import DifficultyAdjustmentsRepository from '../repositories-cash/DifficultyAdjustmentsRepository';
import PricesRepository from '../repositories-cash/PricesRepository';
import priceUpdater from '../tasks-cash/price-updater';
...
class Blocks {
  ...
  private async $getBlockExtended(block: IEsploraApi.Block, transactions: TransactionExtended[]): Promise<BlockExtended> {
    ...
    if (block.height === 0) {
      ...
      // Impotant note: With BCHN the feerates are actually returned in BCH/B, so we multiple by 10^8 to get Sat/B.
      ...
      const feeratePercentiles = stats.feerate_percentiles.map((value) => value * multiplier);
      blockExtended.extras.medianFee = feeratePercentiles[2]; // 50th percentiles
      blockExtended.extras.feeRange = [stats.minfeerate * multiplier, feeratePercentiles, stats.maxfeerate * multiplier].flat();
      blockExtended.extras.totalFees = stats.totalfee * multiplier;
      blockExtended.extras.avgFee = stats.avgfee * multiplier;
      blockExtended.extras.avgFeeRate = stats.avgfeerate * multiplier;
    }
