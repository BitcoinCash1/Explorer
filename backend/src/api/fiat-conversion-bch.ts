  ...
  private async updateCurrency(): Promise<void> {
    ...
    while(retry < config.MEMPOOL.EXTERNAL_MAX_RETRY) {
      try {
        ...
        const response: AxiosResponse = await axios.get(`${fiatConversionUrl}/rates/BCH`, axiosOptions);
        ...
        for (const symbol in rates) {
          if (this.debasingFiatCurrencies.includes(symbol)) {
            this.conversionRates[symbol] = Math.round(100 * rates[symbol]) / 100;
          }
        }
