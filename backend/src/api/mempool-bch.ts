...
import { TransactionExtended, BytesPerSecond } from '../mempool-bch.interfaces';
...
import bitcoinSecondClient from './bitcoin-cash/bitcoin-second-client';
  ...
  private updateTxPerSecond() {
    ...
    if (this.bytesPerSecondArray.length) {
      this.bytesPerSecond = Math.round(
        this.bytesPerSecondArray.map((data) => data.size).reduce((a, b) => a + b) / config.STATISTICS.TX_PER_SECOND_SAMPLE_PERIOD
      );
    }
  }
