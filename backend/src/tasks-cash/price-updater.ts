...
class PriceUpdater {
  ...
  public async $run(): Promise<void> {
    ...
    try {
      ...
      logger.err(`Cannot save BCH prices in db. Reason: ${e instanceof Error ? e.message : e}`);
    }
  ...
  private async $insertHistoricalPrices(): Promise<void> {
    ...
    const pricesJson: any[] = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'tasks', 'mtgox-weekly.json')).toString());
