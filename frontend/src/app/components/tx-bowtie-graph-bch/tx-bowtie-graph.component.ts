...
import { Outspend, Transaction } from '../../interfaces-bch/electrs.interface';
...
@Component({
  selector: 'tx-bowtie-graph-bch',
  templateUrl: './tx-bowtie-graph.component.html',
  styleUrls: ['./tx-bowtie-graph.component.scss'],
})
export class TxBowtieGraphComponentBch implements OnInit, OnChanges {
  ...
  initGraph(): void {
    ...
    this.middle = {
      path: `M ${this.width / 2 - this.midWidth} ${this.height / 2 + 0.5} L ${
        this.width / 2 + this.midWidth
      } ${this.height / 2 + 0.5}`,
      style: `stroke-width: ${this.combinedWeight + 1}; stroke: ${
        this.gradient[1]
      }`,
    };
  }
  ...
  calcTotalValue(tx: Transaction): number {
    const totalOutput = this.tx.vout.reduce(
      (acc, v) => (v.value == null ? 0 : v.value) + acc,
      0
    );
    ...
    if (!this.isLiquid) {
      ...
      const totalInput = this.tx.vin.reduce(
        (acc, v) => (v?.prevout?.value == null ? 0 : v.prevout.value) + acc,
        0
      );
      const confidentialInputCount = this.tx.vin.reduce(
        (acc, v) => acc + (v?.prevout?.value == null ? 1 : 0),
        0
      );
      const confidentialOutputCount = this.tx.vout.reduce(
        (acc, v) => acc + (v.value == null ? 1 : 0),
        0
      );
