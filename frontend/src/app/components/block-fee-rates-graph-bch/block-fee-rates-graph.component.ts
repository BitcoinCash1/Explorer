...
@Component({
  selector: 'app-block-fee-rates-graph-bch',
...
export class BlockFeeRatesGraphComponentBch implements OnInit {
      ...
      tooltip: {
        ...
        formatter: function (data) {
          ...
          for (const rate of data.reverse()) {
            tooltip += `${rate.marker} ${rate.seriesName}: ${rate.data[1]} sats/Byte<br>`;
          }
      ...
      legend:
        data.series.length === 0
          ...
          : {
              data: data.legends,
              selected: JSON.parse(
                this.storageService.getValue('fee_rates_legend')
              ) ?? {
                Min: true,
                '10th': true,
                '25th': true,
