...
@Component({
  selector: 'app-hashrate-chart-bch',
...
export class HashrateChartComponentBch implements OnInit {
    ...
    this.hashrateObservable$ = this.radioGroupForm
      ...
      .valueChanges.pipe(
        ...
        switchMap((timespan) => {
          ...
          this.isLoading = true;
          return this.apiService.getHistoricalHashrate$(timespan).pipe(
            tap((response) => {
                    ...
                    diffFixed.push({
                      ...
                      difficulty:
                        data.difficulty.length > 0
                          ? data.difficulty[data.difficulty.length - 1]
                              .difficulty
                          : null,
                    });
                    ++hashIndex;
          ...
          for (const tick of ticks) {
            if (tick.seriesIndex === 0) {
              ...
              if (this.isMobile()) {
                ...
                hashrate = Math.round(
                  tick.data[1] / hashratePowerOfTen.divider
                );
              }
              hashrateString = `${tick.marker} ${
                tick.seriesName
              }: ${formatNumber(hashrate, this.locale, '1.0-0')} ${
                hashratePowerOfTen.unit
              }H/s<br>`;
              ...
              if (difficulty === null) {
                ...
                if (this.isMobile()) {
                  ...
                  difficulty = Math.round(
                    tick.data[1] / difficultyPowerOfTen.divider
                  );
                }
                difficultyString = `${tick.marker} ${
                  tick.seriesName
                }: ${formatNumber(difficulty, this.locale, '1.2-2')} ${
                  difficultyPowerOfTen.unit
                }<br>`;
              }
