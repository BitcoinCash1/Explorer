...
export class MempoolBlocksComponentBch implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.stateService.isTabHidden$.subscribe(
  ...
  getStyleForMempoolBlock(mempoolBlock: MempoolBlock, index: number) {
    ...
    gradientColors.forEach((color, i, gc) => {
      backgroundGradients.push(`
        ...
        #${color} ${
        (Math.round(((i + 1) / gradientColors.length) * 100) * usedBlockSpace) /
          100 +
        emptyBackgroundSpacePercentage
      }%
      `);
  ...
  calculateTransactionPosition() {
    ...
    for (
      ...
      txInBlockIndex < this.mempoolBlocks.length && !found;
      txInBlockIndex++
    ) {
      const block = this.mempoolBlocks[txInBlockIndex];
      for (let i = 0; i < block.feeRange.length - 1 && !found; i++) {
        if (
          this.txFeePerVSize < block.feeRange[i + 1] &&
          this.txFeePerVSize >= block.feeRange[i]
        ) {
