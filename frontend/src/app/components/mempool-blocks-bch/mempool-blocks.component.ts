...
import { MempoolBlock } from '../../interfaces-bch/websocket.interface';
...
import { DifficultyAdjustment } from '../../interfaces-bch/node-api.interface';

@Component({
  selector: 'app-mempool-blocks-bch',
  templateUrl: './mempool-blocks.component.html',
  styleUrls: ['./mempool-blocks.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class MempoolBlocksComponentBch implements OnInit, OnDestroy {
          ...
          .pipe(
            map((mempoolBlocks) => {
              if (!mempoolBlocks.length) {
                return [{ index: 0, blockSize: 0, feeRange: [0, 0], medianFee: 0, nTx: 0, totalFees: 0 }];
              }
  ...
  getStyleForMempoolBlock(mempoolBlock: MempoolBlock, index: number) {
    const emptyBackgroundSpacePercentage = Math.max(100 - mempoolBlock.blockSize / this.stateService.blockSize * 100, 0);
    ...
    for (let txInBlockIndex = 0; txInBlockIndex < this.mempoolBlocks.length && !found; txInBlockIndex++) {
      ...
      for (let i = 0; i < block.feeRange.length - 1 && !found; i++) {
        if (this.txFeePerVSize < block.feeRange[i + 1] && this.txFeePerVSize >= block.feeRange[i]) {
          ...
          const blockedFilledPercentage = (block.blockSize > this.stateService.blockSize ? this.stateService.blockSize : block.blockSize) / this.stateService.blockSize;
