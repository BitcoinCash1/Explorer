...
@Component({
  selector: 'app-blocks-list-bch',
  templateUrl: './blocks-list.component.html',
  styleUrls: ['./blocks-list.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class BlocksListBch implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.indexingAvailable) {
      this.auditScoreSubscription = this.fromHeightSubject
        .pipe(
          switchMap((fromBlockHeight) => {
            return this.apiService
              .getBlockAuditScores$(
                this.page === 1 ? undefined : fromBlockHeight
              )
              .pipe(
                catchError(() => {
                  return EMPTY;
        ...
        .subscribe((scores) => {
          Object.values(scores).forEach((score) => {
            this.auditScores[score.hash] =
              score?.matchRate != null ? score.matchRate : null;
          });
        });

      this.latestScoreSubscription = this.stateService.blocks$
        .pipe(
          switchMap((block) => {
            if (block[0]?.extras?.matchRate != null) {
              ...
              block[0]?.id &&
              this.auditScores[block[0].id] === undefined
            ) {
              return this.apiService.getBlockAuditScore$(block[0].id).pipe(
                catchError(() => {
                  return EMPTY;
                })
              );
