...
import {
  ...
  Vin,
  Vout,
} from '../../interfaces-bch/electrs.interface';
...
@Component({
  selector: 'app-transactions-list-bch',
  templateUrl: './transactions-list.component.html',
  styleUrls: ['./transactions-list.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TransactionsListComponentBch implements OnInit, OnChanges {
  network = '';
  nativeAssetId =
      ...
      this.transactions.forEach((tx) => {
        ...
        if (this.address) {
          ...
          const addressOut = tx.vin
            .filter(
              (v: Vin) =>
                v.prevout && v.prevout.scriptpubkey_address === this.address
            )
