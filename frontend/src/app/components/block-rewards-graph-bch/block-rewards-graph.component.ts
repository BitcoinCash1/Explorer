...
@Component({
  selector: 'app-block-rewards-graph-bch',
...
export class BlockRewardsGraphComponentBch implements OnInit {
          ...
          return this.apiService.getHistoricalBlockRewards$(timespan).pipe(
            tap((response) => {
              this.prepareChartOptions({
                ...
                blockRewardsUSD: response.body
                  .filter((val) => val.USD > 0)
                  .map((val) => [
                    val.timestamp * 1000,
                    (val.avgRewards / 100000000) * val.USD,
                    val.avgHeight,
                  ]),
              });
        ...
        formatter: function (data) {
          ...
          for (const tick of data) {
            if (tick.seriesIndex === 0) {
              ...
              tooltip += `${tick.marker} ${tick.seriesName}: ${formatCurrency(
                tick.data[1],
                this.locale,
