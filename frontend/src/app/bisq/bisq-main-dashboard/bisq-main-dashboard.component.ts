...
export class BisqMainDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.usdPrice$ = this.stateService.conversions$
      .asObservable()
      .pipe(map((conversions) => conversions.USD));
    ...
    this.tickers$ = combineLatest([
      ...
      map(([tickers, markets, volumes]) => {
        ...
        for (const t in tickers) {
          if (this.stateService.env.BASE_MODULE !== 'bisq') {
            ...
            if (
              pair[1] === 'btc' &&
              this.allowCryptoCoins.indexOf(pair[0]) === -1
            ) {
              continue;
            }
          }
          ...
          mappedTicker.name = `${
            mappedTicker.market.rtype === 'crypto'
              ? mappedTicker.market.lname
              : mappedTicker.market.rname
          } (${
            mappedTicker.market.rtype === 'crypto'
              ? mappedTicker.market.lsymbol
              : mappedTicker.market.rsymbol
          }`;
      ...
      map(([sort, tickers]) => {
        if (sort === 'trades') {
          tickers.sort(
            (a, b) =>
              ((b.volume && b.volume.num_trades) || 0) -
              ((a.volume && a.volume.num_trades) || 0)
          );
        } else if (sort === 'volumes') {
          tickers.sort(
            (a, b) =>
              ((b.volume && b.volume.volume) || 0) -
              ((a.volume && a.volume.volume) || 0)
          );
        } else if (sort === 'name') {
      ...
      map(([trades, markets]) => {
        if (this.stateService.env.BASE_MODULE !== 'bisq') {
          trades = trades.filter((trade) => {
            ...
            return !(
              pair[1] === 'btc' && this.allowCryptoCoins.indexOf(pair[0]) === -1
            );
          });
        }
        return trades
